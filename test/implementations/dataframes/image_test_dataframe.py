...
dataframe1 = MapOrchestrator(CompositeMapper([TypeMapper(LoadedImageColumn), TensorMapper()])).process(dataframe,
                                                                                                     ["input"])
plt.imshow(torch.tensor(dataframe1.get("input'").values[1]).permute(1, 2, 0))
plt.show()

dataframe = MapOrchestrator(CompositeMapper([TypeMapper(LoadedImageColumn), ResizeMapper((50, 50)), GrayScaleMapper(),
                                            TensorMapper()])).process(dataframe, ["input"])
plt.imshow(dataframe.get("input'").values[1].permute(1, 2, 0))
plt.show()
