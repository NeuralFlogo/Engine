...
from pytorch.datasets.transformers.ImageNameNumpyTransformer import ImageNameNumpyTransformer
from pytorch.datasets.transformers.NumericNumpyTransformer import NumericNumpyTransformer
from pytorch.datasets.transformers.WordsTransformer import WordsTransformer
...
def numbers_source_type_csv(path, parameters, batch_size):
    ...
    y = data[:, 0].astype(str)
    dataset = NumericNumpyTransformer(x, y, parameters)
...
def images_source_type(size, mean, std, path, batch_size):
    ...
    dataset = ImageNameNumpyTransformer(transformer, path)
    return __get_loader(dataset, batch_size)


def text_source_type(paths):
    dataset = WordsTransformer(paths)
